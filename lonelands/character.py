...
are no skills and no weapon proficiencies — specialisation lives in a Path's
nodes (``perks.py``). Levelling is frequent and mostly automatic: each level
grants +HP and a periodic +to-hit. **Level 1 is pathless**; from **level 2**
every level also grants **one Path point** (ADR 0011), and the player commits to
a single Path. Data-only; the live sheet lives on the Hero component."""
...
# A Path point every level from level 2 (level 1 is pathless — ADR 0011).
PATH_POINT_FROM_LEVEL = 2
